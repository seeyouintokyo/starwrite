...
Starwrite 是编排层。除下列按需模块外，核心链路模块必须预装；任务开始时逐项检查，缺失时提示安装，不静默降级。

**必装（核心链路）**

| 模块 | 用途 | 缺失后果 |
| --- | --- | --- |
| UniFuncs | 实时搜索、网页阅读、深度搜索、深度研究 | 选题缺少可靠素材时无研究能力，不写正文 |
| human-writing | 中文成文与改稿，定稿须过其 `check_prose.py` | 不写正文 |
| gzh-design-skill | Markdown 到公众号内联 HTML | 不生成公众号排版 |
| html-ppt-skill | 文章到录屏 HTML Slides | 不生成录屏 HTML |
| post-to-wechat 或 Agent 原生公众号 API 能力 | 上传封面、正文图并创建普通草稿 | 不执行发布 |

**按需加载**

| 模块 | 触发条件 |
| --- | --- |
| video-transcript | 输入含视频、播客或本地音视频时；首次模型下载须征得同意 |
| khazix-writer | 仅用户明确指定个人文风时 |
